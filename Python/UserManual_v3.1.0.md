...
> Pada v3.1.0, nilai Threshold dan Hysteresis tetap dimasukkan sebagai Volt agar kompatibel dengan proses kalibrasi, lalu dikonversi ke Kg saat akuisisi dimulai.
...
```
         ┌──────────────┐  pressure_kg ≥ (threshold_kg − hysteresis_kg)
         │    ARMED     │ ─────────────────────────────────► COLLECTING
         └──────────────┘
               ▲
...
**Rumus runtime v3.1.0:**
- `threshold_kg = threshold_volt × scale`
- `hysteresis_kg = hysteresis_volt × scale`
- `lower_trip_kg = threshold_kg − hysteresis_kg`
- `pressure = rata-rata(n_collect sampel yang sudah dalam Kg)`
