# Manual Pengguna — Swimmer Force Motion Monitoring v2.0.0

Dokumen ini menjelaskan pemakaian aplikasi desktop **Swimmer Force Motion Monitoring** (berkas utama: `Swimmer_Force_Motion_Monitoring_v2.0.0.py`) untuk memantau beban dan orientasi (roll, pitch) perenang melalui koneksi serial, merekam data ke CSV, serta menganalisis rekaman dengan **spektrum frekuensi** dan ekspor statistik yang diperluas.
...
Jalankan aplikasi dari folder `Swimmer_Force_Motion_Monitoring_v2.0.0` (atau dengan path penuh):

```text
python Swimmer_Force_Motion_Monitoring_v2.0.0.py
```
...
- **Analisa** — muat file CSV hasil rekaman Live, plot waktu penuh dengan marker ekstremum, **tiga plot spektrum** (FFT atau Welch PSD), kartu statistik (termasuk **frekuensi dominan** per kanal), ekspor ringkasan ke `DataStatistik/`.
...
- `analyze_single_file_tab.py` — implementasi tab Analisa.
...
- **About** — menampilkan dialog informasi aplikasi dan versi (**2.0.0**).
- **Help** — membuka berkas **`UserManual_Force_Motion_v2.0.0.pdf`** dengan aplikasi PDF bawaan sistem. Jika PDF belum ada, dialog menjelaskan cara membuatnya (lihat bagian 6).
...
| `UserManual_Force_Motion_v2.0.0.md` | Manual ini (Markdown) |
| `UserManual_Force_Motion_v2.0.0.pdf` | Manual ini (PDF, opsional) |
...
Tanpa opsi, skrip bawaan masih mengarah ke manual **v1.0.0** di folder yang sama; untuk v2.0.0 gunakan `-i` dan `-o` seperti di atas.
...
| Help tidak membuka PDF | Jalankan §6; pastikan `UserManual_Force_Motion_v2.0.0.pdf` ada di folder aplikasi v2.0.0. |
...
- **Manual:** selaras dengan aplikasi **v2.0.0**.
- Ringkasan perubahan dari v1.0.0 ada di docstring berkas `Swimmer_Force_Motion_Monitoring_v2.0.0.py` (bagian *Changelog*).
