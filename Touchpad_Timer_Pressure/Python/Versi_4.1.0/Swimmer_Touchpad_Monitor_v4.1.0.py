"""
Swimmer_Touchpad_Monitor_v4.1.0.py — NI DAQ Monitor for Touchpad Swimmer
==============================================================================
...
Change Log 4.1.0 dari 4.0.0
---------------------------
* CSV Log opsional kolom tegangan mentah: checkbox
  «Sertakan AI0/AI1 raw (Volt) di CSV Log» (default nonaktif).
...
* ChannelDetector: region Schmitt (upper/lower trip), timeout region 5 s,
  tekanan = rata-rata ±100 sampel di puncak regional.
...
* Dialog Set Parameter: konfirmasi OK/Cancel (Set As Default, Reset to Default,
  Reset to Factory, Apply). «Reset to Saved» diganti «Reset to Default».
...
---------------------------
* Jalur Live: Kg = scale × V per channel (tanpa intercept).
* Threshold dan hysteresis dimuat sebagai Volt; dikonversi ke Kg (× scale) saat Start.
* ChannelDetector Schmitt trigger + n_collect sampel; hasil rata-rata dalam Kg.
* CSV Log: header `timestamp_s,ai0_kg,ai1_kg`.
* Tab Analisa: overlay multi-file CSV Log dalam satu plot.
* Manual aplikasi: `UserManual_v3.1.0.md/.pdf`.
...
Struktur Kelas
--------------
ChannelDetector  — Schmitt region + peak window mean + hold time per channel.
...
PEAK_HALF_WINDOW = 100    # ± sampel di sekitar puncak regional (hingga 201 titik)
...
USER_MANUAL_PDF = pathlib.Path(__file__).parent / "UserManual_v4.1.0.pdf"
USER_MANUAL_MD = pathlib.Path(__file__).parent / "UserManual_v4.1.0.md"

# Ringkasan aplikasi (selaras dengan UserManual_v4.1.0.md — bagian Pendahuluan)
_ABOUT_APP_BLURB = (
    "NI DAQ Monitor Swimmer Touchpad Monitor v4.1.0 adalah aplikasi desktop untuk "
...
# ─── Channel Detector (Schmitt region + peak mean) ───────────────────────────
class ChannelDetector:
    """Deteksi sentuhan: region Schmitt (naik/turun) + rata-rata di puncak + hold.

    State machine:
        ARMED      → tekanan >= upper_trip (threshold + hysteresis)  → IN_REGION
        IN_REGION  → buffer sampel hingga <= lower_trip (inklusif)
                     atau durasi >= max_region_sec (paksa tutup)     → HOLD + emit
        HOLD       → hold_time selesai AND tekanan < lower_trip     → ARMED
    ...
    def __init__(
        ...
        peak_half_window: int = PEAK_HALF_WINDOW,
        max_region_sec: float = MAX_REGION_SEC,
    ) -> None:
        self.threshold = threshold
        self.hysteresis = hysteresis
        self.hold_time = hold_time
        self.peak_half_window = peak_half_window
        self.max_region_sec = max_region_sec
        self._max_region_samples = max(1, int(max_region_sec * sample_rate))
        self._state = "ARMED"
        self._buf: list[float] = []
        self._t0 = 0.0
        self._region_start_ts = 0.0
        self._t_hold_start = 0.0

    @property
    def upper_trip(self) -> float:
    ...
    def _close_region(self, timestamp: float) -> tuple[float, float] | None:
        """Hitung tekanan dari puncak regional, masuk HOLD. Return (t0, pressure)."""
        if not self._buf:
            self._state = "HOLD"
        ...
        ya = np.asarray(self._buf, dtype=float)
        j = int(np.argmax(ya))
        lo = max(0, j - self.peak_half_window)
        hi = min(len(ya) - 1, j + self.peak_half_window)
        pressure = float(np.mean(ya[lo : hi + 1]))
        t0 = self._t0
        self._buf = []
        self._state = "HOLD"
        self._t_hold_start = timestamp
    ...
    def process(self, value: float, timestamp: float) -> tuple[float, float] | None:
        ...
        Returns (t0_s, pressure) saat region ditutup, else None.
        """
        if self._state == "ARMED":
            if value >= self.upper_trip:
                self._state = "IN_REGION"
                self._t0 = timestamp
                self._region_start_ts = timestamp
                self._buf = [value]

        elif self._state == "IN_REGION":
            self._buf.append(value)
            elapsed = timestamp - self._region_start_ts
            force_timeout = (
                len(self._buf) >= self._max_region_samples
                or elapsed >= self.max_region_sec
            )
            if value <= self.lower_trip or force_timeout:
                return self._close_region(timestamp)
...
class MainWindow(QMainWindow):
    ...
    def _build_table_panel(self) -> QWidget:
        ...
        self._btn_help.setToolTip(
            "Buka panduan pengguna (UserManual_v4.1.0.pdf, atau .md jika PDF tidak ada)"
        )
    ...
    def _on_about(self) -> None:
        ...
        title = QLabel("<b>Swimmer Monitor</b><br>NI DAQ Monitor &nbsp;v4.1.0")
